...
class StartupConf(BaseConfig):
    ...
    releasever = Option()
    installroot = Option('/')
    config_file_path = Option('/etc/yum.conf')
...
class YumConf(StartupConf):
    ...
    bugtracker_url = Option('the Amazon Web Services EC2 forums at https://forums.aws.amazon.com/forum.jspa?forumID=30. Include the output of the failed yum action as well as the output of "yum repolist"')
...
def readStartupConfig(configfile, root, releasever=None):
    ...
    if releasever is None and startupconf.releasever is None:
...
#def main():
#    mainconf = readMainConfig(readStartupConfig('/etc/yum.conf', '/'))
