...
class easy_install(Command):
    ...
    user_options = [
        ...
        ('install-layout=', None, "installation layout to choose (known values: amzn, posix, unix)"),
    ...
    def initialize_options(self):
        ...
        # enable custom installation, known values: amzn, posix, unix
    ...
    def finalize_options(self):
        ...
        if self.install_layout:
            if not self.install_layout.lower() in ('amzn', 'posix', 'unix'):
...
""")
    ...
    if sys.version[:3] in ('2.3', '2.4', '2.5') or 'real_prefix' in sys.__dict__:
        ...
        local_sitedir_name = 'site-packages'
    ...
    INSTALL_SCHEMES = dict(
        ...
        unix_local = dict(
            install_dir = '$base/local/lib/python$py_version_short/%s' % local_sitedir_name,
        ...
        amzn_system = dict(
            install_dir = '$base/lib/python/%s' % sitedir_name,
            script_dir  = '$base/bin',
        ),
    )
    ...
    def _expand(self, *attrs):
        ...
        if self.prefix or self.install_layout:
            if self.install_layout and self.install_layout.lower() in ('amzn'):
                    scheme_name = "amzn_system"
                    self.prefix = '/usr'
            elif self.install_layout and self.install_layout.lower() in ('posix', 'unix'):
                    scheme_name = "posix"
                    self.prefix = '/usr'
        ...
        if prefix:
            if sys.platform in ('os2emx', 'riscos'):
                ...
                if sys.version[:3] in ('2.3', '2.4', '2.5'):
                    ...
                    sdir = "dist-packages"
                    local_sdir = "site-packages"
                sitedirs.extend(
                    [os.path.join(prefix, "local/lib", "python" + sys.version[:3], local_sdir),
